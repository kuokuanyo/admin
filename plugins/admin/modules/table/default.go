...
// °²³]°Ñ¼Ælist([]map[string]interface{})ªø«×¤j©ó¹s«h¦^¶Çlist[0](map[string]interface{})
...
func (tb DefaultTable) GetDataWithId(param parameter.Parameters) (FormInfo, error) {
	...
	var (
		...
		// PK³z¹L°Ñ¼Æ__pk´M§äParameters.Fields[__pk]¬O§_¦s¦b¡A¦pªG¦s¦b«h¦^¶Ç²Ä¤@­Óvalue­È(string)¨Ã¥B¥Î","©î¸Ñ¦¨[]string¡A¦^¶Ç²Ä¤@­Ó¼Æ­È
		id      = param.PK()
	)

	// getDataRes°²³]°Ñ¼Ælist([]map[string]interface{})ªø«×¤j©ó¹s«h¦^¶Çlist[0](map[string]interface{})
	if tb.getDataFun != nil {
...
// GetNewForm(¨ú±o·sªí³æ)§PÂ_±ø¥ó(TabGroups)«á¡A³]¸mFormInfo(struct)«á¨Ã¦^¶Ç
...
func (tb DefaultTable) sql() *db.SQL {
	// getDataFromDB(±q¸ê®Æ®w¨ú±o¸ê®Æ)§PÂ_±ø¥ó
	if tb.connectionDriver != "" && tb.getDataFromDB() {
		// WithDriverAndConnection±N°Ñ¼Æ³]¸m(connName¡Bconn)¨Ã¦^¶Çsql(struct)
